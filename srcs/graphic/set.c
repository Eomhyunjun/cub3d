...
/*   Updated: 2021/05/24 13:37:29 by heom             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (cal->side == 0)
		cal->perpwalldist = (cal->mapx - all->posx \
		+ (1 - cal->stepx) / 2) / cal->raydirx;
	else
		cal->perpwalldist = (cal->mapy - all->posy \
		+ (1 - cal->stepy) / 2) / cal->raydiry;
...
void	set_tex_point(t_all *all, t_calc *cal)
{
	...
	cal->tex = (int)(cal->wallx * (double)TEXWIDHT);
	if (cal->side == 0 && cal->raydirx > 0)
		cal->tex = TEXWIDHT - cal->tex - 1;
	if (cal->side == 1 && cal->raydiry < 0)
		cal->tex = TEXWIDHT - cal->tex - 1;
	cal->step = 1.0 * TEXHEIGHT / cal->lineheight;
	cal->texpos = (cal->drawstart - all->height / 2 \
	+ cal->lineheight / 2) * cal->step;
}
